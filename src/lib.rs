...
impl DNSHeader {
    fn to_bytes(&self) -> [u8; 12] {
        let id_bytes: [u8; 2] = self.id.to_be_bytes();
        let flags_bytes: [u8; 2] = self.flags.to_be_bytes();
        let num_questions_bytes: [u8; 2] = self.num_questions.to_be_bytes();
        let num_authorities_bytes: [u8; 2] = self.num_authorities.to_be_bytes();
        let num_additionals_bytes: [u8; 2] = self.num_additionals.to_be_bytes();

        let mut result = [0u8; 12];
        result[..2].copy_from_slice(&id_bytes);
        result[2..4].copy_from_slice(&flags_bytes);
        result[4..6].copy_from_slice(&num_questions_bytes);
        result[6..8].copy_from_slice(&num_authorities_bytes);
        result[8..10].copy_from_slice(&num_additionals_bytes);
